 ...
 const Moves = {
	...
	gravity: {
		...
		condition: {
			...
			durationCallback(source, effect) {
				...
				if (_optionalChain([source, 'optionalAccess', _17 => _17.hasAbility, 'call', _18 => _18('orbital')])) {
                    this.add('-activate', source, 'ability: Orbital', effect);
                    return 999;
                }
				if (_optionalChain([source, 'optionalAccess', _19 => _19.hasItem, 'call', _20 => _20('gravityrock')])) {
					return 8;
				}
				return 5;
			},
	...
	psychicterrain: {
		...
		condition: {
			duration: 5,
			durationCallback(source, effect) {
				if (_optionalChain([source, 'optionalAccess', _34 => _34.hasItem, 'call', _35 => _35('terrainextender')])) {
					return 8;
				}
				return 5;
			},
			...
			onStart(battle, source, effect) {
				if (_optionalChain([effect, 'optionalAccess', _36 => _36.effectType]) === 'Ability') {
	...
	pursuit: {
		...
		onModifyMove(move, source, target) {
			if (_optionalChain([target, 'optionalAccess', _37 => _37.beingCalledBack])) move.accuracy = true;
		},
		onTryHit(target, pokemon) {
	...
	reflect: {
		...
		condition: {
			duration: 5,
			durationCallback(target, source, effect) {
				if (_optionalChain([source, 'optionalAccess', _38 => _38.hasItem, 'call', _39 => _39('lightclay')])) {
					return 8;
				}
				return 5;
			},
	...
	safeguard: {
		...
		condition: {
			duration: 5,
			durationCallback(target, source, effect) {
				if (_optionalChain([source, 'optionalAccess', _40 => _40.hasAbility, 'call', _41 => _41('persistent')])) {
					this.add('-activate', source, 'ability: Persistent', effect);
					return 7;
				}
				return 5;
	...
	suckerpunch: {
		...
		onTry(source, target) {
			const action = this.queue.willMove(target);
			const move = _optionalChain([action, 'optionalAccess', _42 => _42.choice]) === 'move' ? action.move : null;
	...
	thunder: {
		...
		flags: {protect: 1, mirror: 1},
		onModifyMove(move, pokemon, target) {
			switch (_optionalChain([target, 'optionalAccess', _45 => _45.effectiveWeather, 'call', _46 => _46()])) {
			case 'raindance':
			case 'primordialsea':
				move.accuracy = true;
				break;
	...
	trickroom: {
		...
		condition: {
			duration: 5,
			durationCallback(source, effect) {
				if (_optionalChain([source, 'optionalAccess', _47 => _47.hasAbility, 'call', _48 => _48('persistent')])) {
					this.add('-activate', source, 'ability: Persistent', effect);
					return 7;
				}
				return 5;
	...
	voiceoftheforest: {
		...
		condition: {
			duration: 5,
			durationCallback(source, effect) {
				if (_optionalChain([source, 'optionalAccess', _52 => _52.hasItem, 'call', _53 => _53('terrainextender')])) {
					return 8;
				}
				return 5;
			},
			...
			onStart(battle, source, effect) {
				if (_optionalChain([effect, 'optionalAccess', _54 => _54.effectType]) === 'Ability') {
					this.add('-fieldstart', 'move: Grassy Terrain', '[from] ability: ' + effect, '[of] ' + source);
				} else {
					this.add('-fieldstart', 'move: Grassy Terrain');
				}
	...
	rockseal: {
        ...
        condition: {
            ...
            durationCallback(target, source) {
                if (_optionalChain([source, 'optionalAccess', _55 => _55.hasItem, 'call', _56 => _56('gripclaw')])) return 8;
            ...
            onTrapPokemon(pokemon) {
                if (_optionalChain([this, 'access', _57 => _57.effectData, 'access', _58 => _58.source, 'optionalAccess', _59 => _59.isActive])) pokemon.tryTrap();
            },
        },
        secondary: null,
        target: "normal",
	...
	callout: {
        ...
        condition: {
            duration: 1,
            onStart(target, source, effect) {
                if (_optionalChain([effect, 'optionalAccess', _60 => _60.id]) === 'zpower') {
