 ...
 const BattleMovedex = {
	...
	lightscreen: {
		...
		effect: {
			duration: 5,
			durationCallback(target, source, effect) {
				if (_optionalChain([source, 'optionalAccess', _20 => _20.hasItem, 'call', _21 => _21('lightclay')])) {
					return 8;
				}
				return 5;
			},
	...
	magiccoat: {
		...
		effect: {
			...
			onStart(target, source, effect) {
				...
				if (_optionalChain([effect, 'optionalAccess', _22 => _22.effectType]) === 'Move') {
	...
	magicroom: {
		...
		effect: {
			duration: 5,
			durationCallback(source, effect) {
				if (_optionalChain([source, 'optionalAccess', _23 => _23.hasAbility, 'call', _24 => _24('persistent')])) {
					this.add('-activate', source, 'ability: Persistent', effect);
					return 7;
				}
				return 5;
	...
	mistyterrain: {
		...
		effect: {
			duration: 5,
			durationCallback(source, effect) {
				if (_optionalChain([source, 'optionalAccess', _25 => _25.hasItem, 'call', _26 => _26('terrainextender')])) {
					return 8;
				}
				return 5;
			},
			...
			onStart(battle, source, effect) {
				if (_optionalChain([effect, 'optionalAccess', _27 => _27.effectType]) === 'Ability') {
	...
	pursuit: {
		...
		onModifyMove(move, source, target) {
			if (_optionalChain([target, 'optionalAccess', _31 => _31.beingCalledBack])) move.accuracy = true;
		},
		onTryHit(target, pokemon) {
	...
	reflect: {
		...
		effect: {
			duration: 5,
			durationCallback(target, source, effect) {
				if (_optionalChain([source, 'optionalAccess', _32 => _32.hasItem, 'call', _33 => _33('lightclay')])) {
					return 8;
				}
				return 5;
			},
	...
	round: {
		...
		onTry(target, source, move) {
			for (const action of this.queue) {
				...
				// @ts-ignore
				if (_optionalChain([action, 'access', _34 => _34.move, 'optionalAccess', _35 => _35.id]) === 'round') {
					// @ts-ignore
					this.queue.prioritizeAction(action, move);
					return;
				}
	...
	wideguard: {
		...
		effect: {
			...
			onTryHit(target, source, move) {
				...
				if (_optionalChain([move, 'optionalAccess', _43 => _43.target]) !== 'allAdjacent' && move.target !== 'allAdjacentFoes') {
					return;
				}
				if (move.isZ || move.isMax) {
	...
	wonderroom: {
		...
		effect: {
			duration: 5,
			durationCallback(source, effect) {
				if (_optionalChain([source, 'optionalAccess', _44 => _44.hasAbility, 'call', _45 => _45('persistent')])) {
					this.add('-activate', source, 'ability: Persistent', effect);
					return 7;
				}
				return 5;
